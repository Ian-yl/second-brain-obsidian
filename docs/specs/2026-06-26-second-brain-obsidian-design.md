...
- **统一一份**：一份画像 + 一套知识，各 agent（Claude Code / Codex）共享同一 vault。
...
├── references/
│   ├── vault-format.md            # vault 写入规范（画像/知识/索引/CLAUDE.md+AGENTS.md 格式 + 去重 + 四层归类）
...
├── .obsidian/  用户画像.md  CLAUDE.md  AGENTS.md  _索引.md
├── Knowledge/{Inputs,Process,Outputs,Feedback}/<slug>.md   # 四层知识
└── Inbox/                         # 速记收件箱
...
### 3.2 知识 `Knowledge/<层>/<slug>.md`
四层（Inputs / Process / Outputs / Feedback）。frontmatter（`type/layer/created/updated/aliases/tags/sources`）+ `# 标题` + 正文（可含 `[[链接]]`）。
- 原子化（一主题一笔记）；按 slug 跨层匹配既有则合并（`sources`/`tags` 取并集、正文不堆叠重复段落），否则新建。

### 3.3 索引 `_索引.md`（MOC）+ `CLAUDE.md` / `AGENTS.md`
`_索引.md`：按四层分组列笔记 + dataview 聚合。`CLAUDE.md` / `AGENTS.md`（vault 根·两份同内容）= 画像摘要；Claude Code 原生读 `CLAUDE.md`、Codex 读 `AGENTS.md`，作为默认的读取注入。

## 4. 核心流程（全 agent-native）

### 4.1 建库（一次）
agent 问用户库放哪 → 按 `vault-format.md` 建 vault 骨架：`Knowledge/{四层}` + `Inbox` + `.obsidian/app.json` + 空 `用户画像.md` / `CLAUDE.md` + `AGENTS.md` / `_索引.md`。
...
**默认开·本地自动（两模式，初始化时让用户选）**：跑 `install.py --mode end|stop`——`end`=SessionEnd 读整段一次提炼（省）；`stop`=Stop 每条回复后增量提炼（进度标记防重复，实时/更稳）。后台用 `claude -p`/`codex exec`（带 `--strict-mcp-config`，不加载 MCP）读对话、按 vault-format 提炼写 vault；**任何话题都收**（含搭建本系统、项目、闲聊），仅明文密钥/token 脱敏。**纯本地、零外传**；需要 Python + `claude`/`codex` CLI（§0 自动备齐）。

### 4.4 分析会话（深度补充）
- **Claude Code**：用户在输入框打 `/insights 分析我的会话`（斜杠命令只有用户能触发）→ 报告落盘 `~/.claude/usage-data/report-*.html` → agent 把报告文本直接提炼入库。
- **Codex（无 `/insights`）**：会话沉淀走在场提炼（§4.3，任何 agent 都行）；要回顾分析，用户把要提炼的内容贴给 agent / 指一个文件，agent 提炼入库。
- 不读原始 transcript，只提炼用户给的内容 / `/insights` 产出。

### 4.5 知识库维护
- 喂 URL：agent 用 WebFetch 抓正文 → 提炼 → 写对应层笔记。
- 整理收件箱：读 `Inbox/*.md` → 分类提炼写进 `Knowledge/<层>/` → 原速记移到 `Inbox/_archived/`。
- 体检：扫所有笔记查断链 / 孤儿 / 空笔记 → 报告写 `_体检.md`。

### 4.6 读取
vault 内原生自动读 `CLAUDE.md`（Claude Code）/ `AGENTS.md`（Codex）；任何项目里用户说「加载第二大脑」→ agent 读 `用户画像.md` + 相关笔记注入上下文。
...
| 会话分析源 | Claude Code `/insights`；Codex 在场提炼（都不读本地 transcript）|
