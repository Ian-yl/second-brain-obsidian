...
- **按价值分三档**（治噪音 + 防丢）：高价值 → 自动写不问；价值不高 / 拿不准 → 宁可不写、或顺口问一句「这条要存吗？」（别攒一堆批量问）；无价值 → 跳过。用户设了 `confirm_before_write` 则全部写前确认。
- **页脚仪式 + 断点写入（治"忙起来忘提炼"）**：每条实质回复结尾固定带一行 `🧠 第二大脑：记了X / 待提炼N条 / 本轮无新增`——让漏提炼**每轮可见**（用户当那道检查）；忙复杂任务时把要点记进 `~/.second-brain-obsidian/pending.md`，到断点（任务 / 话题收尾）再 flush 写库。
- **取舍**：仍是 best-effort（不像 hook 机械必跑），但页脚把"静默漏"变"可见漏"、断点写入避免跟主任务抢注意力；换来零依赖、不被误判蠕虫。用户也可「更新第二大脑」即时 flush。
...
### 4.7 知识导入（建库后 · 快速把【你的】知识灌进来）
建库 + 采访后**【必问·等答】**要不要现在快速灌知识：① **费曼抽取**（问你懂 / 在学的领域 → 费曼法逼你讲 → 写成你自己的知识笔记、盲区标待补）② **导入你的资料**（URL / 书单 / 文件夹）。**方法论（费曼 / 切片…）是 agent 的建知识手法**（`references/methodologies/`），不是往用户库塞的内容——用户库只装用户自己的知识；用户明确要学习法参考才落 `70-Assets/参考`。完整方案见 `docs/specs/2026-07-01-knowledge-bootstrap-plan.md`。
...
- **没配密钥时**：agent 明确提示配**两个 key**——**听**=Azure（必需）、**读**=MiniMax（推荐·朗读更自然，不配才回退浏览器朗读），别只提 Azure；或先走文字。
