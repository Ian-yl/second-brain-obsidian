...
全程 agent 用自带工具（读写文件 / WebFetch）直接维护 vault；**Python 只用于语音问答**。
...
## 5. 实时语音问答（唯一用 Python 的功能）

- 栈：**STT = 微软 Azure Speech**（短期 token，浏览器端 SDK 实时识别 zh-CN）；**TTS = MiniMax**（服务端 `/tts` 出 mp3）→ 失败回退浏览器 TTS。
...
| 语音 | Azure STT + MiniMax TTS，打电话式，同步文字 + 对话历史；密钥仅 `secrets.env`；唯一用 Python 的功能 |
