...
`_索引.md`：按四层分组列笔记 + dataview 聚合。`CLAUDE.md` / `AGENTS.md`（vault 根·两份同内容）= 画像摘要；Claude Code 原生读 `CLAUDE.md`、Codex / Hermes 读 `AGENTS.md`，作为默认的读取注入。
...
### 4.6 读取
vault 内原生自动读 `CLAUDE.md`（Claude Code）/ `AGENTS.md`（Codex、Hermes）；任何项目里用户说「加载第二大脑」→ agent 读 `用户画像.md` + 相关笔记注入上下文。
