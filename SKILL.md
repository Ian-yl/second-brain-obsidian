...
**每轮回复后，扫一眼这轮有没有「关于用户、值得长期留」的新信息**——决策 / 观点 / 偏好 / 方法 / 在做的项目 / 知识。**任何话题都算**（含闲聊、搭系统的元对话），别按类型预排除。然后按价值处理：

- **明显有价值 → 直接按 `vault-format.md` 写**（去重、按先例落位、同步画像 / `CLAUDE` / `AGENTS` / `00-Home` / MOC 的规则**都在 vault-format**，照它做）。
- **只写用户明说的事实**；你**推断**的（"他大概偏好 X"）别当事实写——标 `sources: 推断·待确认` 或先问一句。
- **拿不准价值 → 宁可不写**，或顺口问一句「这条要存吗？」**别攒一堆批量问。**
- **没价值（寒暄 / 噪声）→ 跳过。** 密钥 / token 一律脱敏不写。
- 写完**顺带提一句记了啥**（方便你喊停）；用户说「更新第二大脑」时一并补全。
- 用户设了 `~/.second-brain-obsidian/config.json` 的 `confirm_before_write: true` → 写前都先确认。

> ⚠️ 没有后台 hook——**全靠你每轮认真过这一遍**，别因忙主任务跳过。多会话同跑时，改共享文件（画像 / `00-Home` / MOC）前**先重读最新再改**，别盲覆盖。
