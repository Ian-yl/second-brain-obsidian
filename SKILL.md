...
- **只在"这段确实没料"时才不写**（没有关于用户、值得长期留的东西）——那是"这段没料"，不是"这类话题不收"。唯一硬性排除：明文密钥 / token → 脱敏不写。
- **默认自动写、不问「要不要存 / 存哪」**：判断到有长期价值 → **直接按 `vault-format.md` 写进 vault**。落位有自动判位（判不出进 `00-Inbox/`），不存在"存哪"歧义；问了只会打断用户——**自动入库就是第二大脑的本分**。你本就有对话上下文，直接写，别等用户开口。
