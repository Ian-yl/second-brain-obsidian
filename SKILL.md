...
description: >-
  ...
  **纯本地个人工具，不联网外传、不自我传播**；vault 读写零依赖，仅可选的语音问答用 Python。
  ...
  做人格问答、分析我的会话、加载第二大脑、设置语音密钥、收录链接、喂URL、整理收件箱、体检知识库、导入知识、灌方法论、快速建知识库。
---
...
- **⑤ 【必做·别直接进日常】问知识起步**：库刚建好还没知识——**必须问一句**「你的第二大脑建好了 🎉，现在库里还没知识，要不要**快速灌点东西打底**？① 方法论种子包 ② 喂链接 / 书单 ③ 费曼法帮你记一个领域」（详见 §知识导入），**等用户选**；用户说"先不用"才进日常。

## 知识导入（建库后 · 快速起步 · 让空库立刻有干货）
...
- **② 批量导入（把存量搬进来）**：
  1. **URL / 书单 / 课程**：用户给一串 → 【逐条】WebFetch 抓正文（书 / 课无正文 → 问「你记得的核心要点是什么」）→ 按 `vault-format.md` 判位入库（判层 + 模式 + 去重）。
  2. **文件夹 / 现有笔记**：用户指路径 → 读 → 按 vault-format 重构入库（一篇内容多 → 用**切片法**拆成多篇原子笔记）；原文件不动。
  3. **知识访谈**：**【必问·等答】**「你最懂 / 在学的 3-5 个领域是啥？」→ 每个走模式③（费曼）→ 秒变笔记。
- **③ 费曼 / 切片抽取（学习法当手法·也用于采访 / 提炼）**：
  1. **费曼式**：问「用大白话给我讲讲你懂的 <主题>？」→ 讲清的写成笔记（`sources: 用户明说`）；含糊 / 跳过的 → 笔记里标 `> 待补：<盲区>`（`sources: 推断·待确认`，别当事实）→ 页脚报「记了 X + N 个待补盲区」。
  2. **切片式**：大主题 → 帮用户**切成原子笔记**（每片一句话说清）+ `[[]]` 互链 → 逐片确认后入库。
- **④ 类型模板**：导入 / 建知识笔记时按类型套 `SK/references/note-templates.md`（方法论 / 概念 / 决策 / 复盘 / 读书笔记 / 项目）——结构统一、可检索。
