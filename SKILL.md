...
- **不预先忽略任何对话**：每段都过一遍——**含搭建 / 调试本系统的元对话、技术讨论、项目、闲聊**，都可能藏着关于用户的真实信息（决策、偏好、在做的事、踩过的坑、方法）。**别因为"是闲聊 / 是元对话"就整段跳过。**
- **按价值分三档处理**（核心判断 · 治噪音 + 防丢）：
  - **① 高价值** = 明确的决策 / 观点 / 偏好 / 方法 / 在做的项目 / 重要事实 → **直接自动写、不问**（落位走自动判位，判不出进 `00-Inbox/`；"存哪"无歧义，问了只打断用户——自动入库就是本分）。
  - **② 价值不高 / 拿不准** = 模糊、一次性、边角、可能有用也可能没用 → **别擅自写、也别直接丢**：攒着，**找个自然的点**（话题告一段落 / 攒了几条 / 准备收尾，**别每轮都打断、也别拖到没机会问**）**一次性问**用户「这几条价值一般，要存进库吗？」——用户挑哪条就存哪条，不要的丢掉。
  - **③ 无价值** = 纯寒暄 / 客套 / 工具噪声 / 无信息量 → 跳过，不问不写。
- **硬性排除**：任何档里的明文密钥 / token → 脱敏，绝不写明文。
- **唯一例外 · 用户要确认**：仅当用户**明确说过**「写库前先问我 / 我先审一下 / 别自动写」时，才在写前给**一句简短确认**（如「这条归到 `20-Process/项目交付/`，写进去？」）点头再写。该偏好存 `~/.second-brain-obsidian/config.json`（`"confirm_before_write": true`）跨会话生效；**没设过 = 一律自动**。写库前先读一眼这个标志。
- 用户说「更新第二大脑」时一并补全。

> ⚠️ **没有后台自动提炼**（那要装 hook + Python，本版去掉了）——第二大脑能不能长起来，**全看你每轮有没有认真过这一遍**。别因为在忙主任务就跳过；这是唯一的入库方式。深度回顾走 `/insights`（§C）。

## 写入纪律（防噪音 #3 / 防冲突 #4 / 可审计 #5 / 防漂移 #6）

每次往 vault 写之前 / 之后，守这几条，避免库越长越乱：

- **#6 按先例落位（防判位漂移）**：写一条知识**前先搜** vault 有没有**同主题 / 同实体**的笔记——**有就跟它放进同一个 `<层>/<中文模式>/`**（沿用先例），别每次重新主观判。关于同一个客户 / 项目 / 实体的笔记，都收在它**第一次落的那个位置**。判一次、后续跟随 → 跨会话 / 跨 agent 才一致。
- **#3 先搜再写、能并就并（防噪音）**：写知识前在目标目录按**标题 / 别名**搜，有近似的 → **合并进去**（`tags`/`aliases`/`sources` 取并集、正文只补不重复的段落），**不新建第二篇**。画像**只记稳定、反复出现**的特质，一次性的别塞进画像。
- **#5 分清「明说」与「推断」（防写错）**：只把**用户明确说过的事实**当 ①高价值自动写；你**推断 / 演绎**出来的（"他大概偏好 X"）→ 降到 **②档问用户**，别当事实写。笔记 frontmatter 的 `sources` 标清 `用户明说` 或 `推断·待确认`。
- **#5 写完留痕（可审计）**：每次自动写完，用**一句话**告诉用户记了啥（如 `📝 入库：<标题> → 30-Outputs/内容生产/`），**别静默**——用户随时能看见、能喊"删了重写"。
- **#4 多会话防撞（防冲突）**：改**共享文件**（`用户画像.md` / `00-Home.md` / `50-MOCs/*`，多会话会同时碰的）前——
  1. 先看 `~/.second-brain-obsidian/.write-lock`：有 **2 分钟内**的活锁（别的 session 在写）→ 本次**跳过共享文件更新**（独立知识笔记照写，等下次再补索引）；
  2. 自己改前**重读该文件最新内容**再改（read-modify-write，别用旧版盖掉别人刚写的）；
  3. 自己改时**置锁**（写入自己的 session 标记 + 时间戳），改完**删锁**；锁 **>5 分钟**视为陈旧、直接忽略。
  - 知识笔记是**各自独立的 `.md`**，并发写不同笔记天然不冲突，无需锁；锁只为护着那几个共享文件。
